...
export default function NewProjectPage() {
  ...
  return (
    <div className="flex min-h-screen bg-[#f5f6f8] text-slate-900 dark:bg-[#0b0f14] dark:text-slate-100">
      <MainSidebar />
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 overflow-auto px-6 py-8 md:px-8">
          <div className="mx-auto grid max-w-7xl gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(17rem,19.5rem)]">
            <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_24px_80px_rgba(5,10,20,0.08)] dark:border-white/8 dark:bg-[#171b22] dark:shadow-[0_24px_80px_rgba(5,10,20,0.35)]">
              <div className="mb-6">
                <p className="text-[11px] font-bold uppercase tracking-[0.32em] text-brand">
                  New Workspace
                </p>
                <h1 className="mt-3 text-balance text-2xl font-black tracking-tight text-slate-900 dark:text-white sm:text-3xl">
                  운영 단위를 분리할 프로젝트를 만드세요
                </h1>
                <div className="mt-2 space-y-1 text-sm leading-relaxed text-slate-500 dark:text-slate-400 sm:leading-7 xl:text-[15px]">
              ...
              <form className="grid gap-6" onSubmit={onSubmit}>
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(16rem,0.9fr)]">
                  <label className="grid gap-2">
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">프로젝트 이름</span>
                    <input
                      value={name}
                      onChange={(event) => {
                      ...
                      className="h-14 rounded-2xl border border-slate-200 bg-slate-50 px-4 text-base text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-brand/60 focus:ring-2 focus:ring-brand/15 dark:border-white/10 dark:bg-[#0f1319] dark:text-white dark:placeholder:text-slate-500"
                    />
                  ...
                  <label className="grid gap-2">
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">AWS 리전</span>
                    <select
                      value={awsRegion}
                      onChange={(event) => setAwsRegion(event.target.value)}
                      className="h-14 rounded-2xl border border-slate-200 bg-slate-50 px-4 text-base text-slate-900 outline-none transition focus:border-brand/60 focus:ring-2 focus:ring-brand/15 dark:border-white/10 dark:bg-[#0f1319] dark:text-white"
                    >
                    ...
                    <span className="text-xs text-slate-400 dark:text-slate-500">{selectedRegion.description}</span>
                  </label>
                </div>
                ...
                <div className="rounded-[24px] border border-slate-200 bg-slate-50 p-5 dark:border-white/8 dark:bg-[#0f1319]">
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-2 text-pretty text-sm leading-relaxed text-slate-500 dark:text-slate-400 sm:leading-7 xl:text-[15px]">
                    ...
                    <div className="w-full shrink-0 rounded-2xl border border-brand/20 bg-brand/10 px-4 py-4 lg:w-auto lg:min-w-[27rem]">
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-orange-700 dark:text-[#8fb5ff]">
                        생성 후 바로 할 일
                      </p>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-2 dark:border-white/8">
                  <button
                    type="button"
                    onClick={() => router.back()}
                    className="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-300 hover:bg-slate-100 dark:border-white/10 dark:text-slate-300 dark:hover:border-white/20 dark:hover:bg-white/5"
                  >
                    이전으로
                  </button>
                  <button
            ...
            <aside className="min-w-0 rounded-[28px] border border-slate-200 bg-slate-50 px-4 py-5 shadow-[0_24px_80px_rgba(5,10,20,0.06)] dark:border-white/8 dark:bg-[#131720] dark:shadow-[0_24px_80px_rgba(5,10,20,0.28)]">
              <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-slate-400 dark:text-slate-500">
                Guide
              </p>
              <h2 className="mt-2 text-lg font-black tracking-tight text-slate-900 dark:text-white xl:text-xl xl:whitespace-nowrap">
                프로젝트 분리 가이드
              </h2>
              ...
              <div className="mt-4 space-y-2.5">
                {[
                  ...
                  <div key={item.title} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 dark:border-white/8 dark:bg-[#0f1319]">
                    <h3 className="text-[13px] font-bold leading-snug text-slate-900 dark:text-white">{item.title}</h3>
                    <p className="mt-1.5 text-[12px] leading-5 text-slate-500 dark:text-slate-400">{item.body}</p>
                  </div>
                ))}
              </div>
            </aside>
